...
public:
	...
	*/
	...
	/* Indicate that either the entire window or just specific regions of it must be redrawn */
	void Redraw( bool bAllWindow) { m_bNeedsRedraw = true; if ( bAllWindow) m_bWholeWindowUpdate = true; }

	/* Get (possibly loading) a specific font. If already loaded it will just give you the existing pointer. */
	...
	/* 
		...
		Basically, some functions may maintain a reference into the array and need to know if the thing they point to was destroyed.
	...
	*/
	void DrawableObjectsChanged() { m_bDrawableObjectsChanged = true; }
...
public:
	...
	*/
	...
	Must call Redraw(true/false) if anything changes that shows on the window.
	*/
	...
	*/
	...
	/* Get a pointer to the SDL surface for the window */
	SDL_Surface * GetSDLSurface() { return m_pForegroundSurface; }

	// Get a pointer to the foreground surface
	SDL_Surface * GetForeground() { return m_pForegroundSurface; }
...
public:
	...
	/* Copy the background pixels in the specified region onto the window */
	...
	*/
	...
	inline void SafeSetForegroundPixel( int iX, int iY, unsigned int uiColour )
	{
		SafeSetPixel( iX, iY, uiColour, m_pForegroundSurface );
	}
	...
	inline void SafeSetPixel( int iX, int iY, unsigned int uiColour, SDL_Surface* pTarget )
	{
		...
		if ( (iX < 0) || (iX >= GetIntsPerWindowRow( pTarget ) ) || (iY < 0) || (iY >= m_iWindowHeight) )
			return;

		// Set the value of the pixel
		((unsigned int *)pTarget->pixels)[iX + iY * GetIntsPerWindowRow( pTarget ) ] = uiColour;
	}
	...
	inline unsigned int SafeGetForegroundPixel(int iX, int iY) const
	{
		return SafeGetPixel(iX, iY, m_pForegroundSurface);
	}
	...
	inline unsigned int SafeGetPixel(int iX, int iY, SDL_Surface* pTarget) const
	{
		...
		return ((unsigned int *)pTarget->pixels)[iX + iY * GetIntsPerWindowRow( pTarget )];
	}

	/* Method to set a pixel colour on the window, fast and in-lined. */
	inline void SetWindowPixel( int iX, int iY, unsigned int uiColour )
	{
		assert( iX >= 0 );
		assert( iX < m_iWindowWidth);
		assert( iY >= 0 );
		assert( iY < m_iWindowHeight);
		SetPixel( iX, iY, uiColour, m_pForegroundSurface );
	}
	...
	{
		assert( iX >= 0 );
		assert( iX < GetIntsPerWindowRow( pTarget ) );
		assert( iY >= 0 );
		assert( iY < m_iWindowHeight);
		// Set the value of the pixel
		((unsigned int *)pTarget->pixels)[iX + iY * GetIntsPerWindowRow( pTarget )] = uiColour;
	...
	void DrawForegroundLine(int iX1, int iY1, int iX2, int iY2, unsigned int uiColour)
	{
		DrawLine( iX1, iY1, iX2, iY2, uiColour, m_pForegroundSurface );
	}
	...
	void DrawForegroundThickLine(int iX1, int iY1, int iX2, int iY2, unsigned int uiColour, int iThickness)
	{
		DrawThickLine( iX1, iY1, iX2, iY2, uiColour, iThickness, m_pForegroundSurface );
	}
	...
	{
		DrawVerticalSidedRegion( fX1, fX2, fY1a, fY2a, fY1b, fY2b, uiColour, m_pForegroundSurface );
	}
	...
	void DrawForegroundTriangle(
		...
		unsigned int uiColour )
	{
		DrawTriangle( iX1, iY1, iX2, iY2, iX3, iY3, uiColour, m_pForegroundSurface );
	}
	...
	void DrawForegroundRectangle(int iX1, int iY1, int iX2, int iY2, unsigned int uiColour)
	{
		DrawRectangle( iX1, iY1, iX2, iY2, uiColour, m_pForegroundSurface );
	}
	...
	void DrawForegroundOval(int iX1, int iY1, int iX2, int iY2, unsigned int uiColour)
	{
		DrawOval( iX1, iY1, iX2, iY2, uiColour, m_pForegroundSurface );
	}
		...
		double fX3, double fY3, double fX4, double fY4, unsigned int uiColour )
	{
		...
		DrawPolygon( 4, pXArray, pYArray, uiColour, m_pForegroundSurface );
	}

	// Draw a polygon on the Foreground
	void DrawForegroundPolygon( double fX1, double fY1, double fX2, double fY2,
		...
		double fX5, double fY5, unsigned int uiColour )
	{
		...
		DrawPolygon( 5, pXArray, pYArray, uiColour, m_pForegroundSurface );
	}
		...
		double fX5, double fY5, double fX6, double fY6, unsigned int uiColour )
	{
		...
		DrawPolygon( 6, pXArray, pYArray, uiColour, m_pForegroundSurface );
	}
		...
		double fX7, double fY7, unsigned int uiColour )
	{
		...
		DrawPolygon( 7, pXArray, pYArray, uiColour, m_pForegroundSurface );
	}
		...
		double fX7, double fY7, double fX8, double fY8, unsigned int uiColour )
	{
		...
		DrawPolygon( 8, pXArray, pYArray, uiColour, m_pForegroundSurface );
	}

	// Draw a polygon on the Foreground
	void DrawForegroundPolygon( int iPoints, double* pXArray, double* pYArray, unsigned int uiColour )
	{
		DrawPolygon( iPoints, pXArray, pYArray, uiColour, m_pForegroundSurface );
	}
	...
	{
		DrawString( iX, iY, pText, uiColour, pFont, m_pForegroundSurface );
	}
	...
	void DrawForegroundShortenedArrow( int iX1,int iY1,int iX2,int iY2,int iShortedStart,int iShortenedEnd,unsigned int uiColour,int iThickness,int iHeadSize )
	{
		DrawShortenedArrow( iX1,iY1,iX2,iY2,iShortedStart,iShortenedEnd,uiColour,iThickness,iHeadSize,m_pForegroundSurface );
	}
	...
	void DrawForegroundShortenedLine( int iX1,int iY1,int iX2,int iY2,int iShortedStart,int iShortenedEnd,unsigned int uiColour,int iThickness )
	{
		DrawShortenedLine( iX1,iY1,iX2,iY2,iShortedStart,iShortenedEnd,uiColour,iThickness,m_pForegroundSurface );
	}
...
protected:
	...
	*/
	...
	since it always used the actual window surface.
	int m_iIntsPerwindowRow; 
	Was set to :
	m_iIntsPerwindowRow = m_pForegroundSurface->pitch / sizeof(unsigned int);
	*/
	int GetIntsPerWindowRow(SDL_Surface* pSurface) const
	{
	...
	{
		return m_pForegroundSurface->pitch / sizeof(unsigned int);
	}
	...
	/* True if whole window should be updated on the next draw, false if only the moving objects should. */
	bool m_bWholeWindowUpdate;
